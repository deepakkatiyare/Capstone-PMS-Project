...
export class AppointmentService {
  ...
  public getAppointmentByStatusAndId(id: any, status: any, index: any, size: any): Observable<AppointmentDto[]> {
    return this.http.get<AppointmentDto[]>("http://localhost:9003/appointment/" + id + "/" + status + "/" + index + "/" + size);
  }
  ...
  public getPreviousPatientAppointments(patientId: any): Observable<AppointmentDto[]> {
    return this.http.get<AppointmentDto[]>('http://localhost:9003/appointment/' + patientId + '/completed');
  }

  public getAppointmentByPatientId(id: any): Observable<AppointmentDto> {
    return this.http.get<AppointmentDto>("http://localhost:9003/patient-appointment/" + id);
  }

  public getAppointmentCount(email: any) {
    return this.http.get("http://localhost:9003/appointment-count/" + email);
  }
  public getPendingAppointmentCount(email: any) {
    return this.http.get("http://localhost:9003/pending-count/" + email);
  }
  public getAcceptedAppointmentCount(email: any) {
    return this.http.get("http://localhost:9003/accepted-count/" + email);
  }

  public getCompletedAppointmentCountByPatientId(id: any) {
    return this.http.get("http://localhost:9003/completed/appointment/patient/" + id);
  }
